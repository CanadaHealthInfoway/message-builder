...
public class EdValidationUtils {
	...
	public void doValidate(EncapsulatedData encapsulatedData, String specializationType, Hl7BaseVersion baseVersion, String type, Hl7Errors errors) {
	...
	public void doValidate(String specializationType, 
		...
		if (StandardDataType.ED_DOC_OR_REF.getType().equals(type) && !Hl7BaseVersion.CERX.equals(baseVersion)) {
			if (StringUtils.isBlank(specializationType) || StandardDataType.ED.getType().equals(specializationType)) {
				...
				type = (content == null || content.length == 0 ? StandardDataType.ED_DOC_REF.getType() : StandardDataType.ED_DOC.getType());
				createError("Invalid specializationType: " + specializationType + ". The specializationType must be ED.DOC or ED.DOCREF for ED.DOC_OR_REF types. Value will be treated as " + type + ".", element, errors);
			} else {
		...
		if (hasCompression) {
			if (StandardDataType.ED_REF.getType().equals(type)) {
				...
				createError("Only GZ compression allowed for CeRx ED.DOCORREF type.", element, errors);
