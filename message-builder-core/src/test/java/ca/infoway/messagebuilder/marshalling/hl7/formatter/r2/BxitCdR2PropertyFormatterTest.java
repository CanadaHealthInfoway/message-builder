...
public class BxitCdR2PropertyFormatterTest extends FormatterTestCase {

	@Test
	public void testNullFlavor() throws Exception {
		String result = new BxitCdR2PropertyFormatter().format(getContext("name", "BXIT<CD>"), new BXITImpl<CodedTypeR2<Code>>(NullFlavor.NO_INFORMATION));
	...
	public void testEverythingSpecified() throws Exception {
		...
		assertEquals(3, this.result.getHl7Errors().size());
		assertXml("result", 
				"<name code=\"cm\" codeSystem=\"1.2.3.4\" codeSystemName=\"aCodeSystemName\" codeSystemVersion=\"aCodeSystemVersion\" displayName=\"aDisplayName\" qty=\"1122\">" +
				"<originalText>some original text</originalText>" +
				"<qualifier inverted=\"true\"><name code=\"cm\" codeSystem=\"1.2.3.4\"/><value code=\"normal\" codeSystem=\"2.16.840.1.113883.5.14\"/></qualifier>" +
