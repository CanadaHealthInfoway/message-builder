...
public class RealConfPropertyFormatterTest {
	...
	public void testFormatCorrectly() throws Exception {
		...
		Map<String, String> result = new RealConfPropertyFormatter().getAttributeNameValuePairs(new FormatContextImpl(new ModelToXmlResult(), null, "name", null, null), new BigDecimal(realValue), null);
		assertEquals("map size", 1, result.size());
