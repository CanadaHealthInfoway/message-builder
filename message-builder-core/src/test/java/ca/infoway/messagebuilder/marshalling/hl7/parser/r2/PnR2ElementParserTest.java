...
public class PnR2ElementParserTest extends MarshallingTestCase {
	...
	private ParseContext createContext(String type, VersionNumber version) {
		return ParseContextImpl.create(type, PersonName.class, version, null, null, ConformanceLevel.POPULATED, null, null);
	}
	...
	public void testParsePrefixes() throws Exception {
        ...
        personName = (PersonName) new PnR2ElementParser().parse(createContext("PN", SpecificationVersion.R02_04_02), node, this.xmlResult).getBareValue();
		assertTrue(this.xmlResult.isValid());
	...
	@Test
    public void testParseAllWithDisallowedQualifier() throws Exception {
		Node node = createNode(
				"<name>" +
		...
		PersonName personName = (PersonName) new PnR2ElementParser().parse(createContext("PN", SpecificationVersion.R02_04_02), node, this.xmlResult).getBareValue();
		assertFalse(this.xmlResult.isValid());
		assertEquals("number of errors", 1, this.xmlResult.getHl7Errors().size());
