/**
 * Copyright 2013 Canada Health Infoway, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *        http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * Author:        $LastChangedBy: tmcgrady $
 * Last modified: $LastChangedDate: 2011-05-04 15:47:15 -0400 (Wed, 04 May 2011) $
 * Revision:      $LastChangedRevision: 2623 $
 */
/* This class was auto-generated by the message builder generator tools. */
package ca.infoway.messagebuilder.model.nb_pr_cr_lr.common.coct_mt090310nb;

import ca.infoway.messagebuilder.annotation.Hl7PartTypeMapping;
import ca.infoway.messagebuilder.annotation.Hl7RootType;
import ca.infoway.messagebuilder.annotation.Hl7XmlMapping;
import ca.infoway.messagebuilder.datatype.II;
import ca.infoway.messagebuilder.datatype.ST;
import ca.infoway.messagebuilder.datatype.impl.IIImpl;
import ca.infoway.messagebuilder.datatype.impl.STImpl;
import ca.infoway.messagebuilder.datatype.lang.Identifier;
import ca.infoway.messagebuilder.model.MessagePartBean;



/**
 * <p>Business Name: EHR Repository</p>
 * 
 * <p>Provides context about the record and its management.</p>
 * 
 * <p>Identification of the EHR infostructure responsible for 
 * the storage and management of the record.</p>
 */
@Hl7PartTypeMapping({"COCT_MT090310NB.AssignedDevice"})
@Hl7RootType
public class EHRRepositoryBean extends MessagePartBean {

    private static final long serialVersionUID = 20150902L;
    private II id = new IIImpl();
    private ST assignedRepositoryName = new STImpl();
    private ST representedRepositoryJurisdictionName = new STImpl();


    /**
     * <p>Business Name: Repository Identifier</p>
     * 
     * <p>Relationship: COCT_MT090310NB.AssignedDevice.id</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Allows repositories to be uniquely identified for linking 
     * or grouping purposes and is therefore mandatory.</p>
     * 
     * <p>A unique identifier for the EHR repository.</p>
     * 
     * <p>The &ldquo;id&rdquo; shall always be as follows:</p> <ul> 
     * <li>For Client Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.3.6&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> <li>For Provider 
     * Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.4.10.2&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> <li>For Location 
     * Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.5.10.2&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> </ul>
     */
    @Hl7XmlMapping({"id"})
    public Identifier getId() {
        return this.id.getValue();
    }

    /**
     * <p>Business Name: Repository Identifier</p>
     * 
     * <p>Relationship: COCT_MT090310NB.AssignedDevice.id</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Allows repositories to be uniquely identified for linking 
     * or grouping purposes and is therefore mandatory.</p>
     * 
     * <p>A unique identifier for the EHR repository.</p>
     * 
     * <p>The &ldquo;id&rdquo; shall always be as follows:</p> <ul> 
     * <li>For Client Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.3.6&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> <li>For Provider 
     * Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.4.10.2&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> <li>For Location 
     * Registry messages: &ldquo;&lt;id 
     * root=&quot;2.16.124.4.101.35.5.10.2&quot; 
     * use=&quot;BUS&quot;/&gt;&rdquo;</li> </ul>
     */
    public void setId(Identifier id) {
        this.id.setValue(id);
    }


    /**
     * <p>Business Name: Repository Name</p>
     * 
     * <p>Relationship: COCT_MT090310NB.Repository.name</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Provides a human-readable name for the repository and is 
     * therefore mandatory</p>
     * 
     * <p>The name of the repository which is responsible for 
     * maintaining the record. E.g. &quot;Ontario Health 
     * Respository #3&quot;</p>
     * 
     * <p> The &ldquo;name&rdquo; field in the 
     * &ldquo;/assignedDevice/ assignedRepository&rdquo; message 
     * path shall always be as follows:</p> <ul> <li>For Client 
     * Registry messages: &ldquo;NEW BRUNSWICK CLIENT 
     * REGISTRY&rdquo;</li> <li>For Provider Registry messages: 
     * &quot;NEW BRUNSWICK PROVIDER INDEX&quot;</li> <li>For 
     * Location Registry messages: &quot;NEW BRUNSWICK LOCATION 
     * INDEX&quot;</li> </ul>
     */
    @Hl7XmlMapping({"assignedRepository/name"})
    public String getAssignedRepositoryName() {
        return this.assignedRepositoryName.getValue();
    }

    /**
     * <p>Business Name: Repository Name</p>
     * 
     * <p>Relationship: COCT_MT090310NB.Repository.name</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Provides a human-readable name for the repository and is 
     * therefore mandatory</p>
     * 
     * <p>The name of the repository which is responsible for 
     * maintaining the record. E.g. &quot;Ontario Health 
     * Respository #3&quot;</p>
     * 
     * <p> The &ldquo;name&rdquo; field in the 
     * &ldquo;/assignedDevice/ assignedRepository&rdquo; message 
     * path shall always be as follows:</p> <ul> <li>For Client 
     * Registry messages: &ldquo;NEW BRUNSWICK CLIENT 
     * REGISTRY&rdquo;</li> <li>For Provider Registry messages: 
     * &quot;NEW BRUNSWICK PROVIDER INDEX&quot;</li> <li>For 
     * Location Registry messages: &quot;NEW BRUNSWICK LOCATION 
     * INDEX&quot;</li> </ul>
     */
    public void setAssignedRepositoryName(String assignedRepositoryName) {
        this.assignedRepositoryName.setValue(assignedRepositoryName);
    }


    /**
     * <p>Business Name: Repository Jurisdiction Name</p>
     * 
     * <p>Relationship: COCT_MT090310NB.RepositoryJurisdiction.name</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Establishes business context for determining 
     * custodianship, and is therefore mandatory.</p>
     * 
     * <p>The name of the jurisdiction that is responsible for the 
     * EHR infostructure that contains and manages the record.</p>
     * 
     * <p>The &ldquo;name&rdquo; field in the 
     * &ldquo;/assignedDevice/ 
     * respresentedRepositoryJurisdiction&rdquo; message path shall 
     * always be &ldquo;NEW BRUNSWICK&rdquo; across Client, 
     * Provider and Location registry messages.</p>
     */
    @Hl7XmlMapping({"representedRepositoryJurisdiction/name"})
    public String getRepresentedRepositoryJurisdictionName() {
        return this.representedRepositoryJurisdictionName.getValue();
    }

    /**
     * <p>Business Name: Repository Jurisdiction Name</p>
     * 
     * <p>Relationship: COCT_MT090310NB.RepositoryJurisdiction.name</p>
     * 
     * <p>Conformance/Cardinality: MANDATORY (1)</p>
     * 
     * <p>Establishes business context for determining 
     * custodianship, and is therefore mandatory.</p>
     * 
     * <p>The name of the jurisdiction that is responsible for the 
     * EHR infostructure that contains and manages the record.</p>
     * 
     * <p>The &ldquo;name&rdquo; field in the 
     * &ldquo;/assignedDevice/ 
     * respresentedRepositoryJurisdiction&rdquo; message path shall 
     * always be &ldquo;NEW BRUNSWICK&rdquo; across Client, 
     * Provider and Location registry messages.</p>
     */
    public void setRepresentedRepositoryJurisdictionName(String representedRepositoryJurisdictionName) {
        this.representedRepositoryJurisdictionName.setValue(representedRepositoryJurisdictionName);
    }

}
