...
public class JavaDomainTypeWriter implements SourceFileWriter {

	private final DomainType domainType;

	public JavaDomainTypeWriter(DomainType domainType) {
	...
	public void write(Writer writer) throws IOException {
        ...
        writePackage(getPackageName(), writer);
        writer.write(LINE_SEPARATOR);
	...
	private void writeImports(String packageName, Writer writer) throws IOException {
		...
		if (CollectionUtils.isEmpty(type)) {
			writeImport(packageName, Code.class.getName(), writer);
		} else {
			for (DomainType domainType : type) {
				writeImport(packageName, domainType.getFullyQualifiedClassName(), writer);
			}
		}
		
	}

	private void writeImport(String packageName, String name, Writer writer) throws IOException {
		if (!StringUtils.equals(packageName, ClassUtils.getPackageName(name))) {
			writer.write("import ");
			writer.write(name);
			writer.write(";");
	        writer.write(LINE_SEPARATOR);
	...
	private String getPackageName() {
		return ClassUtils.getPackageName(this.domainType.getFullyQualifiedClassName());
	}
